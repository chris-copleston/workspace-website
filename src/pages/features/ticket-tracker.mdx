...
4.	Direct communication channel
   - Ability for users and assigned service provider to easily communicate over the phone or Microsoft Teams.
    ...
    <div class="tablet relative w-full h-[700px] mx-auto">
      <Tablet
        item={{ img: AdminServicesImage.src, color: ['#ffffff'] }}
        start={{ x: 0, y: -1, z: 2, rotationX: -150, rotationY: 0, rotationZ: 0 }}
        end={{ x: 0, y: -0.8, z: 2, rotationX: 14, rotationY: 0, rotationZ: 0 }}
        enableOrbitControls={false}
        enableAutoRotation={false}
      />
    </div>
