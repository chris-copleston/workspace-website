...
import HomeTabletImage from '~/assets/images/pages/ipad-wallpaper.jpg';
...
4.	Direct communication channel
   - Ability for users and assigned service provider to easily communicate over the phone or Microsoft Teams.
    ...
    <div class="tablet relative w-full h-[600px] mx-auto">
      <Tablet
        item={{ img: HomeTabletImage.src, color: ['#ffffff'] }}
        start={{ x: 0, y: -1, z: 2, rotationX: 45, rotationY: 0, rotationZ: 0 }}
        end={{ x: 0, y: 0, z: 1.5, rotationX: -180, rotationY: 0, rotationZ: 0 }}
        enableOrbitControls={false}
        enableAutoRotation={false}
      />
    </div>
